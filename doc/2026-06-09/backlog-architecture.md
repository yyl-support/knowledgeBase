...
├── services/                        # 服务注册表（19 个 YAML，引擎适配层）
...
└── scripts/                         # Python/Shell 工具脚本（18 个）
...
| 文件 | 行数 | 核心职责 |
|------|------|---------|
| `src/orchestrate.sh` | 1458 | 通用两阶段自动开发引擎，agent 编排 |
| `src/deployer/deploy.sh` | 144 | K8s runtime-clone 预览部署 |
| `src/gates/run.sh` | 40 | 门禁检查→自动修复→重检循环 |
| `src/gates/checks.sh` | 218 | 7 项门禁检查实现 |
| `src/gates/security_gate_local.sh` | 149 | PR 前 security-gate 预跑 |
| `src/tests/run_layered.sh` | 56 | 分层测试/部署调度 |
| `scripts/resolve_service.py` | 246 | 按 Issue label 匹配服务配置 |
