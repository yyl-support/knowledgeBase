...
backlog (平台类)
  │
  ├── command-router (监听 Issue 评论)
  │      → resolve_service (路由匹配服务配置)
  │      → orchestrate.sh (编排 AI Agent)
  │
  └── services/*.yaml (19个服务配置)   ← resolve_service 匹配 / orchestrate.sh 回读
         ├── meeting-server.yaml
         ├── robot.yaml
         └── calculator.yaml
  │
...
分发到三类仓库
  ...
  │
  └──▼ 各业务服务仓库
         ├── forum-reply-robot (机器人类)
         ├── meeting-server (会议服务类)
         ├── calculator (中台类)
         └── 其他微服务... (搜索/平台/中间件)
```
...
- [[backlog-architecture]] — backlog 引擎架构
- [[服务间调用关系]] — 全组织调用关系矩阵（互补）
- [[local-debug-本地调试体系]] — 本地调试体系
- [[术语解释]] — 术语字典
- [[README]] — 组织代码总览
