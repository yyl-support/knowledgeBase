...
> **仓库**：https://github.com/yyl-support/yyl-agents
> **日期**：2026-06-08
...
| 全文搜索 | SQLite FTS5 | 内置，中文分词用 jieba-wasm |
...
| 向量存储   | chromadb                                 | 嵌入式，与 SQLite 并存 |
| 依赖管理   | uv 或 pip + requirements.txt              |                 |
...
│     │
│     ▼
├── HTTP Server (Hono)  :51820
│     └──◄── SSE ──  浏览器 Web UI
│     │
│     ▼
├── SQLite DB · memory.db   ◄── better-sqlite3
│     │   (知识索引 + 会话 + 消息)
│     ▼
├── TS 主进程   localhost:51990
│     └── HTTP(token) ──►  Python 服务 (FastAPI)
│                            ├── · 语义搜索
│                            ├── · 知识摘要
│                            ├── · 相关推荐
│                            └── · Embedding
...
| 层 | 职责 |
|---|---|
| TS 主进程 | MCP 协议、SQLite CRUD、HTTP 路由、Web UI 渲染、启动 Python 子进程、日志管理 |
| Python 增强服务 | 语义搜索（embedding）、知识摘要生成、相关推荐、异步 embedding 写入 |
...
CREATE TABLE knowledge_index (
  ...
  summary         TEXT,                       -- AI 摘要，用于 FTS 全文检索
...
CREATE VIRTUAL TABLE knowledge_fts USING fts5(
  name, task, purpose, description, summary, tags,
...
| `summary` | AI 自动生成的内容摘要（FTS 主索引目标） |
| `tags` | 灵活的标签体系，支持交叉分类 |
...
Agent 调用 knowledge_save
    ...
    ├── 6. 异步调用 Python → 生成 summary + embedding
    ├── 7. 日志记录 → [INFO] knowledge_saved: {path} ({file_size} bytes)
    └── 8. 返回 { id, path }
```
...
CREATE TABLE sessions (
  ...
  agent         TEXT NOT NULL,              -- opencode / claude-code
...
| `knowledge_search` | Knowledge | `{project, query, format?, tags?, mode?}` | `[{id, name, path, format, description, score}]` |
...
yyl-agents start
    ...
    ├── 读取 ~/.yyl-agents/config.json
    ├── 确保项目目录结构存在 (knowledge/{doc,picture,md,other}, skill, log, scripts)
    ├── 初始化日志系统 (pino → log/ 目录)
    ├── [INFO] Starting yyl-agents daemon...
    ├── 启动 TS 主进程 (localhost:51820)
    ├── 若 python.enabled → 启动 Python 子进程 (localhost:51990)
    ├── 等待 Python 就绪 (GET /health)
    ├── 写入 ~/.yyl-agents/daemon.pid
    ├── [INFO] Daemon ready. Web UI: http://localhost:51820
    └── 前台运行，Ctrl+C 或 yyl-agents stop 终止
```
...
Agent → MCP tool call → TS 主进程
    ...
    │
    └── AI 增强操作 → HTTP POST → Python /semantic-search
                    → 合并结果 → 返回 Agent
```
...
使用 **pino**（Node.js 端）+ Python **logging**（Python 端）。
...
| `warn` | Python 服务不可达（降级处理）、同步失败、文件校验异常 |
...
[INFO]  Python service ready (pid=12346, port=51990)
...
[ERROR] Python service unreachable, falling back to FTS5 search
...
```
顶栏:  yyl-agents  ◉ connected   项目: backlog   ⚙ 设置
│
├── 会话列表 (左 1/4)
│     ├── 新建会话
...
├── 知识库 (中 2/4)
│     ├── 搜索:[      ]  🔍语义  📋全部  📄md  ⚙过滤
│     ├── 2026-06-08 · api-redesign
│     │     └── 架构评审意见 · ☁ 已同步 ✅
│     └── 2026-06-07 · bugfix-502
│           └── 根因分析图.png · ☁ 待同步 🖼
│
├── 消息总线 (右 1/4)
│     ├── 发送给:[opencode▼]
...
│     └── 📝 输入消息...  [发送]
│
└── 状态栏:  TS v0.1.0 | Python ✓ | 知识 23条(3待同步) | 消息 5
```

### 9.2 知识库面板功能

- 搜索栏：关键词（FTS5）/ 语义搜索（Python）切换
...
| 命令 | 功能 |
|---|---|
| `yyl-agents start` | 启动 daemon（前台运行） |
| `yyl-agents start -d` | 后台运行（daemonize） |
| `yyl-agents stop` | 停止 daemon |
| `yyl-agents ui` | 打开浏览器 → localhost:51820 |
| `yyl-agents status` | 显示运行状态、知识统计、日志路径 |
| `yyl-agents sync` | 同步未推送知识到远端 GitHub |
| `yyl-agents config` | 打印当前配置 |
| `yyl-agents init` | 初始化项目：创建 knowledge/ skill/ log/ scripts/ 目录，注入 MCP 配置片段 |
| `yyl-agents mcp` | 以 MCP stdio transport 启动（供 agent 调用） |

### 10.2 配置文件

路径：`~/.yyl-agents/config.json`
...
```json
{
  ...
  "database": {
    "globalPath": "~/.yyl-agents/global.db"
  },
  "ui": {
    "theme": "auto"
  },
...
### 10.4 CLAUDE.md / AGENTS.md 注入

`yyl-agents init` 在目标项目根目录执行时：

1. 创建 `knowledge/{doc,picture,md,other}/`、`skill/`、`log/`、`scripts/` 目录
2. 若存在 `AGENTS.md` → 追加 MCP 配置和使用说明
3. 若存在 `CLAUDE.md` → 同上
4. 若都不存在 → 创建 `AGENTS.md`，包含 MCP 配置 + yyl-agents 工具使用规范

CLAUDE.md 作用域说明：仅在 yyl-agents 项目目录下启动的 agent 才能自动加载该文件。对其他项目，需通过 `yyl-agents init` 注入配置。
...
| `/generate-embedding` | POST | `{text}` → 向量 |
...
- TS ↔ Python 通信使用随机生成的 shared secret token
- Web UI API 同样需要 token（嵌入页面中）
...
│   │       └── store.ts
│   ├── python/
│   │   └── client.ts           # Python HTTP 客户端（含 fallback 降级）
│   ├── logger/
...
- [ ] Python 摘要生成使用规则模板还是接入本地小模型
- [ ] 后台运行（-d 模式）的跨平台实现方案
- [ ] embedding 模型首次下载策略（预置？运行时自动？）
- [ ] 多项目间的知识迁移/引用机制
- [ ] FTS5 中文分词方案选型（jieba-wasm vs. 内置 simple tokenizer）
